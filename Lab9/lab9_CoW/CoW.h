...
class CoW
{
	...
	public:
		...
		// Do not store reference to WriteProxy
		T* operator->() const& = delete;
		T* operator->() const&&
		{
			return m_p;
...
public:
	...
	CoW(CoW<T>&& rhs) : m_shared(std::move(rhs.m_shared)) {}

	CoW(std::unique_ptr<T> pUniqueObj) : m_shared(std::move(pUniqueObj)) {}
	...
	template <typename U>
	CoW(CoW<U>& rhs) : m_shared(rhs.m_shared) {}
	...
	CoW(CoW const& rhs) : m_shared(rhs.m_shared) {}
